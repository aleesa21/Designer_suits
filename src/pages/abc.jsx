...
function AdminProducts() {
  ...
  const handleOpenEdit = (product) => {
    setEditingId(product.design_id);
    setFormData({ ...product });
    setIsEditorOpen(true);
  };
  ...
  const handleSave = (e) => {
    ...
    if (editingId) {
      setProducts(
        products.map((p) =>
          p.design_id === editingId ? { ...formData, design_id: editingId } : p,
        ),
      );
    } else {
      setProducts([...products, { ...formData, design_id: Date.now() }]);
    }
  ...
  const handleDelete = (id) => {
    if (confirm("Are you sure you want to remove this design?")) {
      setProducts(products.filter((p) => p.design_id !== id));
    }
  };
  ...
  /* MAIN CATALOG VIEW WITH TRANSPARENT GLASS TABLE CONTAINER */
  return (
